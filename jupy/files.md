...
date: 2026-07-26 11:54:45
root_folder: "jupy"
total_compiled_files: 76
---
...
# Packages installed into every *code* venv for Jupy's own features
...
def ensure_env(env_dir, on_progress=None):
    if os.path.exists(env_dir) and not _venv_is_valid(env_dir):
        if on_progress:
            on_progress(f"Found incomplete env at {env_dir} — recreating…")
        try:
            import shutil
            shutil.rmtree(env_dir)
        except Exception as e:
            raise RuntimeError(
                f"Environment at {env_dir} is incomplete and couldn't be removed: {e}"
            ) from e
    ...
    if not os.path.isfile(python):
        raise RuntimeError(f"Failed to create working Python at {env_dir} (interpreter missing).")
...
def ensure_jupy_dependencies():
    """Ensures Jupy's server-side dependencies (psutil) are installed."""
    missing = []
    ...
    if missing:
        print(f"[Jupy] Installing required packages: {', '.join(missing)}")
        try:
            # Use --no-input to avoid prompts, timeout after 60 seconds
            subprocess.run(
                [sys.executable, "-m", "pip", "install", "--no-input", "-q"] + missing,
                capture_output=True,
                timeout=60,
                check=True
            )
            print("[Jupy] Dependencies installed successfully.")
        except subprocess.TimeoutExpired:
            print("[Jupy] ERROR: pip install timed out after 60 seconds.")
            raise
        except subprocess.CalledProcessError as e:
            print(f"[Jupy] ERROR: pip install failed with code {e.returncode}")
            print(f"stderr: {e.stderr}")
            raise
    else:
        print("[Jupy] All dependencies already installed.")
...
# ---------------------------------------------------------------------------
# Resolving / switching environment
# ---------------------------------------------------------------------------
    ...
    def _ensure_kernel_proc(self):
        with self.lock:
            if self.proc is None or self.proc.poll() is not None:
                ...
                # Start a reader thread to prevent the pipe from blocking
                def reader_thread():
                    while self.proc and self.proc.poll() is None:
                        try:
                            ...
                            # print("[KERNEL] ", line, end="", file=sys.stderr)
                        except:
                            break
...
def _run_magic(line, cell, namespace):
    ...
    if magic_name == 'paste':
        ...
        # Reuse existing implementation (kept for compatibility)
...
# ---- Existing magics (time, timeit, cd, etc.) ----
...
# Lazy kernel import – avoids blocking server startup
_kernel = None
    ...
    def handle_run_ws(self):
        ...
        while True:
            ...
            try:
                ...
                if action == "run":
                    code = req.get("code", "")
                    threading.Thread(target=get_kernel().execute, args=(code, ws_send), daemon=True).start()
                elif action == "interrupt":
                    get_kernel().interrupt()
                elif action == "stdin_reply":
                    val = req.get("value", "")
                    get_kernel().handle_stdin_reply(val)
            except Exception:
                pass
...
.widget-integer-slider .widget-label {
  font-weight: 700;
  min-width: 80px;
}
.widget-integer-slider input[type="range"] {
  flex: 1;
}
.widget-integer-slider .widget-value {
  min-width: 30px;
  font-family: var(--font-mono);
}
